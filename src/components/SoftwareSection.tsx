...
export default function SoftwareSection() {
  ...
  return (
    <div className="space-y-12 animate-fade-in text-[var(--theme-text)]">
      
      {/* Editorial Introduction block */}
      <div className="bg-[var(--theme-bg-card)] border border-[var(--theme-border)] rounded-2xl p-6 md:p-8 space-y-4 shadow-[0_10px_35px_rgba(0,0,0,0.05)] backdrop-blur-md relative overflow-hidden bg-dot-matrix">
        <div className="absolute top-0 right-0 w-32 h-32 bg-[var(--theme-accent)]/5 rounded-full blur-2xl"></div>
        
        <div className="flex items-center justify-between border-b border-[var(--theme-border)] pb-4">
          <div className="flex items-center gap-3">
            <Terminal className="w-5 h-5 text-[var(--theme-accent)]" />
            <h2 className="font-display font-black text-2xl uppercase tracking-tight text-[var(--theme-text-heading)]">
              01_ Engineering Studio
            </h2>
          </div>
          <div className="flex items-center gap-2 text-[9px] font-mono text-[var(--theme-accent)] bg-[var(--theme-glow)] px-2.5 py-1 rounded-full border border-[var(--theme-border)]">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping"></span>
            <span>MATRIX_STABLE</span>
          </div>
        </div>

        <p className="text-sm text-[var(--theme-text-muted)] leading-relaxed max-w-3xl font-normal">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 mt-2">
          <div className="bg-[var(--theme-bg-base)]/50 border border-[var(--theme-border)] rounded-xl p-3.5 space-y-1 text-center font-mono">
            <span className="text-[9px] text-neutral-500 block">RENDER_OPTIMIZATION</span>
            <span className="text-sm font-bold text-[var(--theme-accent)]">140fps / Ultra</span>
          </div>
          <div className="bg-[var(--theme-bg-base)]/50 border border-[var(--theme-border)] rounded-xl p-3.5 space-y-1 text-center font-mono">
            <span className="text-[9px] text-neutral-500 block">BUNDLE_BOUNDS</span>
            <span className="text-sm font-bold text-[var(--theme-accent)]">&lt; 14KB static</span>
          </div>
          <div className="bg-[var(--theme-bg-base)]/50 border border-[var(--theme-border)] rounded-xl p-3.5 space-y-1 text-center font-mono">
            <span className="text-[9px] text-neutral-500 block">FIDELITY_COEFF</span>
            <span className="text-sm font-bold text-[var(--theme-accent)]">0.998 / Dec</span>
          </div>
          <div className="bg-[var(--theme-bg-base)]/50 border border-[var(--theme-border)] rounded-xl p-3.5 space-y-1 text-center font-mono">
            <span className="text-[9px] text-neutral-500 block">INTEGRITY_INDEX</span>
            <span className="text-sm font-bold text-[var(--theme-accent)]">100% Typing</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <span className="text-[10px] uppercase tracking-widest font-mono text-[var(--theme-accent)] font-semibold">// EXHIBITIONS</span>
            <h3 className="font-display font-black text-xl text-[var(--theme-text-heading)] uppercase tracking-tight flex items-center gap-2 mt-1">
              <AppWindow className="w-5 h-5 text-[var(--theme-accent)]" />
      ...
      <div className="space-y-6">
        ...
        <div className="flex bg-[var(--theme-bg-card)] border border-[var(--theme-border)] p-1 rounded-xl text-xs font-mono max-w-md shadow-lg">
          <button
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <span className="text-[10px] uppercase tracking-widest font-mono text-[var(--theme-accent)] font-semibold">// CAPABILITIES</span>
            <h3 className="font-display font-black text-xl text-[var(--theme-text-heading)] uppercase tracking-tight flex items-center gap-2 mt-1">
          ...
          <div className="relative">
            <Search className="absolute left-3.5 top-3 w-4 h-4 text-[var(--theme-accent)]/50" />
            <input
              ...
              className="pl-10 pr-4 py-2 bg-[var(--theme-bg-base)] border border-[var(--theme-border)] rounded-full text-xs font-mono font-medium tracking-wide text-[var(--theme-text)] w-full sm:w-60 focus:outline-none focus:border-[var(--theme-accent)] focus:ring-1 focus:ring-[var(--theme-accent)]/25 transition-all placeholder:text-neutral-500"
            />
          </div>
        </div>

        {/* Dynamic Tabs list tailored with modern rounded design */}
        <div className="flex flex-wrap gap-1.5 border-b border-[var(--theme-border)] pb-3">
          {skillCategories.map((cat) => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-[10px] font-mono tracking-wider transition-all duration-300 cursor-pointer ${
                activeCategory === cat
                  ? "bg-[var(--theme-accent)] text-white dark:text-[#050807] font-bold shadow-[0_0_12px_rgba(16,185,129,0.3)]"
                  : "text-[var(--theme-text-muted)] hover:text-[var(--theme-text-heading)] bg-[var(--theme-glow)] hover:bg-[var(--theme-border)] border border-[var(--theme-border)]"
              }`}
            >
              {cat.toUpperCase()}
            </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredSkills.map((skill) => (
            <div
              ...
              key={skill.name}
              className="bg-[var(--theme-bg-card)] border border-[var(--theme-border)] rounded-xl p-4 space-y-3 hover:border-[var(--theme-accent)]/25 hover:bg-[var(--theme-bg-card)] transition-all duration-300"
            >
              <div className="flex justify-between items-center">
                <span className="font-extrabold text-xs uppercase tracking-tight text-[var(--theme-text-heading)]">{skill.name}</span>
                <span className="font-mono text-[10px] font-bold text-[var(--theme-accent)] bg-[var(--theme-glow)] px-2 py-0.5 rounded border border-[var(--theme-border)]">{skill.level}%</span>
              </div>
              
              {/* Thick Technical emerald progress segment */}
              <div className="w-full bg-[var(--theme-bg-base)] rounded-full h-2 overflow-hidden border border-[var(--theme-border)]">
                <div
                  className="bg-gradient-to-r from-[var(--theme-accent)] to-[var(--theme-accent-hover)] h-full rounded-full transition-all duration-500"
              ...
              <div className="flex justify-between text-[8px] font-mono font-bold text-neutral-500 uppercase">
                ...
                <span className="text-[var(--theme-accent)] align-middle">● ONLINE</span>
              </div>
            </div>
          ))}
      ...
      <div className="bg-[var(--theme-bg-card)] border border-[var(--theme-border)] rounded-2xl p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-[0_12px_30px_rgba(0,0,0,0.02)] relative overflow-hidden bg-tech-grid">
        <div className="absolute top-0 right-0 w-48 h-48 bg-emerald-500/5 rounded-full blur-[80px] pointer-events-none"></div>
        
        <div className="space-y-2 z-10">
          <span className="text-[9px] uppercase font-mono tracking-[0.2em] bg-[var(--theme-glow)] text-[var(--theme-accent)] px-3 py-1 rounded-full border border-[var(--theme-border)]">
            COLLABORATION_PORT_ACTIVE
          </span>
          <h3 className="font-display font-black text-2xl uppercase tracking-tight text-[var(--theme-text-heading)] mt-1">Ready to Optimize Your Interface?</h3>
          <p className="text-xs text-[var(--theme-text-muted)] max-w-xl leading-relaxed font-normal">
        ...
        <button
          ...
          className="px-6 py-3.5 bg-gradient-to-r from-[var(--theme-accent)] to-[var(--theme-accent-hover)] text-white dark:text-[#050807] font-bold text-xs uppercase tracking-widest rounded-full flex items-center justify-center gap-2 cursor-pointer self-start md:self-auto shrink-0 transition-all duration-300 shadow-[0_4px_15px_var(--theme-glow)] hover:opacity-90 hover:scale-105"
        >
          {copiedEmail ? (
            <>
